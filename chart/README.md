...
[![Version: 1.2.1](https://img.shields.io/badge/Version-1.2.1-informational?style=flat-square)](https://github.com/HenriqZimer/dolphin-helm-chart)
...
GameCube/Wii emulator, served as a full desktop over the browser via KasmVNC.
...
```bash
git clone https://github.com/HenriqZimer/dolphin-helm-chart.git
cd dolphin-helm-chart

helm package chart/
helm install dolphin ./dolphin-1.2.1.tgz
```
...
### Dolphin crashes when launching a game (SIGBUS)

If the dashboard/UI works but a game crashes right after boot (check the container logs for
`Unhandled SIGBUS`), Dolphin's JIT recompiler may need more `/dev/shm` than the small
...
starts when `/dev/shm` is too small); not independently confirmed on Dolphin, but worth trying
...
[linuxserver.io GPU Configuration docs](https://docs.linuxserver.io/images/docker-dolphin/#gpu-configuration)):
...
If Dolphin itself crashes with `Unhandled SIGBUS` in the container logs right after boot (i.e. the
...
provided by a third-party [Docker Mod](https://github.com/LoneAngelFayt/dolphin-romm-integration)
...
- `broker_host` is called **server-to-server**, by RomM's own backend. Point
  ...
  `http://dolphin.<namespace>.svc.cluster.local:8000` — cluster-internal DNS
...
| `image.repository` | `lscr.io/linuxserver/dolphin` | Container image. |
